-- Fix "column reference kingdom_id is ambiguous" in place_building_transaction.
--
-- Root cause: the function declares `returns table(kingdom_id uuid, ...)`, which
-- makes `kingdom_id` an OUT parameter in scope for the entire function body.
-- Three inner queries used `kingdom_id` as an unqualified column reference,
-- causing PostgreSQL to raise an ambiguity error between the OUT parameter and
-- the buildings.kingdom_id column.
--
-- Fix: add explicit table aliases and qualify every `kingdom_id` column reference.
