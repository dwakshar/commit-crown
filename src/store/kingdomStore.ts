...
export const useKingdomStore = create<KingdomStore>((set, get) => ({
  ...
  setKingdom: (kingdom) =>
    set({
      ...
      selectedBuilding:
        kingdom.buildings.find((building) => building.id === get().selectedBuilding?.id) ??
  ...
  setBuildings: (buildings) =>
    set((state) => ({
      ...
      selectedBuilding:
        buildings.find((building) => building.id === state.selectedBuilding?.id) ??
        buildings.find((building) => !building.isPlaceholder) ??
        buildings[0] ??
        null,
    })),
  selectBuilding: (selectedBuilding) => set({ selectedBuilding }),
  setBuildModeType: (buildModeType) => set({ buildModeType, placementError: null }),
  updateGold: (gold) =>
    set((state) => ({
  ...
  refreshKingdom: async () => {
    ...
    set((state) => ({
      ...
      selectedBuilding:
        payload.kingdom?.buildings.find((building) => building.id === state.selectedBuilding?.id) ??
        payload.kingdom?.buildings.find((building) => !building.isPlaceholder) ??
  ...
  placeBuilding: async (buildingType, x, y) => {
    ...
    if (!Number.isInteger(x) || !Number.isInteger(y) || x < 0 || x > 19 || y < 0 || y > 19) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Unable to place building'
      set({ placementError: message })
      return null
    } finally {
