  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        className={`relative flex h-11 min-w-11 items-center justify-center overflow-hidden rounded-[16px] border px-3 transition ${
          open
          ...
          <div className="relative border-b border-[var(--b0)] px-4 py-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-[12px] border border-[var(--b0)] bg-[rgba(255,255,255,0.03)] text-[var(--silver-2)] transition hover:border-[var(--b2)] hover:text-[var(--silver-0)]"
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <div className="rounded-[18px] border border-[rgba(200,88,26,0.18)] bg-[rgba(44,21,13,0.42)] px-3 py-2.5">
                <p className="text-[10px] uppercase tracking-[0.2em] text-[var(--silver-3)]">Unread</p>
                <p className="mt-1 font-[var(--font-head)] text-xl text-[var(--silver-0)]">{unreadCount}</p>
              </div>
              <div className="rounded-[18px] border border-[var(--b0)] bg-[rgba(255,255,255,0.03)] px-3 py-2.5">
                <p className="text-[10px] uppercase tracking-[0.2em] text-[var(--silver-3)]">Archive</p>
                <p className="mt-1 font-[var(--font-head)] text-xl text-[var(--silver-0)]">{notifications.length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="relative px-4 py-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                ...
                className="realm-button realm-button-secondary rounded-[14px] px-3 py-2 text-[10px] disabled:cursor-not-allowed disabled:opacity-45"
              >
                Mark all as read
              </button>
            </div>
          ...
          <div className="relative max-h-[min(60vh,460px)] space-y-3 overflow-y-auto px-4 pb-4">
            {notifications.length > 0 ? (
              notifications.map((notification) => (
                <article
                  key={notification.id}
                  className={`rounded-[22px] border bg-[linear-gradient(180deg,rgba(18,23,32,0.96),rgba(8,12,18,0.98))] p-3.5 ${NOTIFICATION_STYLES[notification.type].accent} ${NOTIFICATION_STYLES[notification.type].glow}`}
                >
                  <div className="flex items-start gap-3">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[16px] border border-[var(--b0)] bg-[linear-gradient(180deg,rgba(200,88,26,0.12),rgba(255,255,255,0.02))] font-[var(--font-head)] text-sm text-[var(--silver-0)]">
                      {NOTIFICATION_COPY[notification.type].icon}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <p className={`text-[11px] uppercase tracking-[0.22em] ${NOTIFICATION_STYLES[notification.type].badge}`}>
              ...
              <div className="rounded-[24px] border border-[var(--b0)] bg-[linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0.015))] px-5 py-10 text-center">
                <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-[18px] border border-[var(--b0)] bg-[rgba(255,255,255,0.03)] text-lg text-[var(--silver-2)]">
                  ✦
                </div>
                <p className="mt-4 font-[var(--font-head)] text-[1rem] uppercase tracking-[0.08em] text-[var(--silver-0)]">
                  Quiet battlements
                </p>
                <p className="mt-2 text-sm leading-6 text-[var(--silver-2)]">
                  No messages from the realm yet. Raids, achievements, and visits will appear here.
                </p>
              </div>
            )}
          </div>
