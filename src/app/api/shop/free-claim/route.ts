...
export async function POST(request: Request) {
  ...
  const [{ data: item, error: itemError }, { data: existingOwnership, error: ownershipError }] =
    await Promise.all([
      supabase.from('shop_items').select('id, is_free').eq('id', parsed.data.itemId).maybeSingle(),
      supabase
        .from('owned_items')
        .select('id')
        .eq('user_id', user.id)
  ...
  if (!typedItem.is_free) {
    return NextResponse.json({ error: 'Item is not free' }, { status: 400 })
  }
